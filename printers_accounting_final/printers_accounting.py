...
REMOVED_ITEM_TYPES = {"Картридж", "Тонер"}
...
class App(ctk.CTk):
    ...
    def normalize_lists(self, data):
        ...
        for item_type in data.get("item_types", []):
            if item_type not in REMOVED_ITEM_TYPES:
    ...
    def load_all_data(self):
        ...
        for item in self.items:
            ...
            item.setdefault("type", DEFAULT_ITEM_TYPES[0])
    ...
    def sort_lists(self):
        ...
        self.lists["item_types"] = sorted_values([
            value for value in self.lists.get("item_types", DEFAULT_ITEM_TYPES)
            if value not in REMOVED_ITEM_TYPES
        ])
    ...
    def add_learned_value(self, list_key, value):
        if list_key == "item_types" and value in REMOVED_ITEM_TYPES:
            return False
    ...
    def clear_fields(self, reset_selection=True):
        ...
        self.type_box.set(DEFAULT_ITEM_TYPES[0])
        self.color_box.set("-")
    ...
    def fill_fields(self, item):
        ...
        self.type_box.set(item.get("type", DEFAULT_ITEM_TYPES[0]))
    ...
    def create_backup(self, show_message=True):
        ...
        stamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        copied = 0
        for file in [DATA_FILE, HISTORY_FILE, CATALOG_FILE, LISTS_FILE]:
            if file.exists():
    ...
    def restore_from_backup(self):
        ...
        stamp = None
        source_files = [DATA_FILE, HISTORY_FILE, CATALOG_FILE, LISTS_FILE]
    ...
    def build_settings_tab(self):
        ...
        text = ctk.CTkLabel(
            settings_card,
            text="Здесь можно очистить историю операций и управлять пользовательскими списками программы.",
            font=(APP_FONT, 14),
            justify="left"
        )
